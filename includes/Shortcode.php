...
class Shortcode {
    ...
    public function render( $atts = [] ) : string {
        ...
        <div class="aff-portal" data-aff-portal>
            ...
            <section id="panel-link" class="aff-tabpanel" role="tabpanel" data-tab-panel="link" aria-labelledby="tab-link" hidden>
                ...
                <?php if ( $partner ): ?>
                    ...
                    <p class="aff-hint">Uwaga: tylko adresy w tej samej domenie są dozwolone. <code>?to=</code> przyjmuje ścieżkę względną, <code>?url=</code> pełny adres do tej samej domeny.</p>
                <?php else: ?>
        ...
        <?php
        // Minimalny inline-config dla JS
        $config = [
            'home'      => home_url('/'),
            'base_link' => $base_link,
        ];
        printf(
            '<script type="application/json" id="aff-portal-config">%s</script>',
